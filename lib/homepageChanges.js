...
const generateChangesDoc = (changeset) => {
  ...
  const tableContents = Object.entries(changeset.changes).map(([ts, change]) => {
    ...
    const timeString = new Date(change.ts).toLocaleString('en-CA', { timeZone: 'America/Vancouver' })
    const changes = []
    if (change.hero) {
...
const writeChangesDoc = (changeset) => {
  const docFilename = `./docs/${source}.md`
  console.log('writing changes to %s', docFilename)
